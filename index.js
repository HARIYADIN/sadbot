...
const apibarbar = 'IDxO1TFYnKADlX4pxcHa'; // APIKEY MHANKBARBAR
...
if (text.includes('!pornhub')){
    ...
    axios.get(`https://api.zeks.xyz/api/phlogo?text1=${text1}&text2=${text2}&apikey=${vinzkey}`).then((res) => {
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
...
//RETRO LOGO
  if (text.includes('!retrologo')) {
    var retro = text.split("!retrologo ")[1];
    var text1 = retro.split("&")[0];
    var text2 = retro.split("&")[1];
    var text3 = retro.split("&")[2];
    axios.get(`https://api.zeks.xyz/api/retro?text1=${text1}&text2=${text2}&text3=${text3}&=${vinzkey}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[❗] Sabar bro pasrti kemren nih', MessageType.text, { quoted: m })
            conn.sendMessage(id, buf, MessageType.image, { caption: `Anjay bah :v`, quoted: m });
          })
    })
  }
/*NINJA LOGO
    ...
    axios.get(`https://api.zeks.xyz/api/snowwrite?text1=${text1}&text2=${text2}&apikey=${vinzkey}`)
      .then((res) => {
        imageToBase64(res.data.result)
          .then(
            (ress) => {
              conn.sendMessage(id, '[🔰] Bentar bro gw pergi ke salju nya dumlu yakan...', MessageType.text, { quoted: m })
              var buf = Buffer.from(ress, 'base64')
